          ...
          {myVehicles.length === 0 ? (
            <Card>
              <CardContent className="py-4 text-sm text-muted-foreground">
                You don't have vehicles yet. Press “Add” to create one.
              </CardContent>
            </Card>
          ) : (
            myVehicles.map((v) => (
              <Card key={v.id} className="rounded-xl">
                <CardHeader className="py-3">
                  <CardTitle className="text-base flex items-center justify-between">
                    ...
                    <Badge variant={v.id === currentVehicleId ? "default" : "outline"}>
                      {v.id === currentVehicleId ? "Current" : v.connectorType}
                    </Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-0 pb-3">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>
                      {v.connectorType} · {v.batteryKWh} kWh · typical {v.typicalChargeKW} kW
                    </span>
                  </div>
